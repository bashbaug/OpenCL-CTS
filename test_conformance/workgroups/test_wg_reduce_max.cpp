...
verify_wg_reduce_max_int(int *inptr, int *outptr, size_t n, size_t wg_size)
{
    ...
    for (i=0; i<n; i+=wg_size)
    {
        ...
        for (j=0; j<((n-i) > wg_size ? wg_size : (n-i)); j++)
        {
            if ( max != outptr[i+j] )
            {
                log_info("work_group_reduce_max int: Error at %zu: expected = %d, got = %d\n", i+j, max, outptr[i+j]);
                return -1;
            }
        }
    }
...
verify_wg_reduce_max_uint(unsigned int *inptr, unsigned int *outptr, size_t n, size_t wg_size)
{
    ...
    for (i=0; i<n; i+=wg_size)
    {
        ...
        for (j=0; j<((n-i) > wg_size ? wg_size : (n-i)); j++)
        {
            if ( max != outptr[i+j] )
            {
                log_info("work_group_reduce_max uint: Error at %zu: expected = %d, got = %d\n", i+j, max, outptr[i+j]);
                return -1;
            }
        }
    }
...
verify_wg_reduce_max_long(cl_long *inptr, cl_long *outptr, size_t n, size_t wg_size)
{
    ...
    for (i=0; i<n; i+=wg_size)
    {
        ...
        for (j=0; j<((n-i) > wg_size ? wg_size : (n-i)); j++)
        {
            if ( max != outptr[i+j] )
            {
                log_info("work_group_reduce_max long: Error at %zu: expected = %lld, got = %lld\n", i+j, max, outptr[i+j]);
                return -1;
            }
        }
    }
...
verify_wg_reduce_max_ulong(cl_ulong *inptr, cl_ulong *outptr, size_t n, size_t wg_size)
{
    ...
    for (i=0; i<n; i+=wg_size)
    {
        ...
        for (j=0; j<((n-i) > wg_size ? wg_size : (n-i)); j++)
        {
            if ( max != outptr[i+j] )
            {
                log_info("work_group_reduce_max ulong: Error at %zu: expected = %llu, got = %llu\n", i+j, max, outptr[i+j]);
                return -1;
            }
        }
    }
